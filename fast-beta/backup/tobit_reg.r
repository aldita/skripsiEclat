...
output$tobit_reg <- renderUI ({
  sidebarLayout(
	 ...
	 div(class = "busy",
	  ...
	  wellPanel(
		 bsButton("moTrig", "Organinize properties of Tobit", style = "link")
			 
      ),
	  wellPanel(
		 actionButton('save_residual', 'Save Residual')
	...
	conditionalPanel(condition = "input.conditionedPanels == 'Identification'",
	  wellPanel(
		selectInput(
            ...
            c("None"="none","Censored Variable Plot"="cen","Histogram"="hist","Correletaion"='cor',"Scatter"="scat"), selectize=FALSE)
      )),
    ...
    mainPanel(
    	column(11, HTML(paste("<h5 align=right>",bsButton("moTrig2", "Export Your Output", style = "link"),"</h5>"))),
		HTML(paste("<h5 align=right>",bsButton("share", label=img(src="image/share.png", height = 40, width = 40), style="link"),"</h5>")),
		bsModal1("mo", "Organinize properties of Tobit regression", trigger = "moTrig",
		column(4, wellPanel(  
		...
		selectInput(
            ...
            choices = list(
              "Newton-Raphson" = "nr",
			   ...
			   "Nelder-Mead" ="nm"),"nr",selectize = FALSE
			),
			selectInput(
		...
		textInput("author", strong("Author :") , value = ""),br(),
		selectInput(
		  inputId = "category",
              ...
              div(
                ...
                helpText(
				  ...
				  The AIC and Schwarz Criterion can be used to compare nested and non-nested models.</p>"))
					...
					bsCollapsePanel("Parameter Estimates", uiOutput('param_table'),tableOutput('margEff'), tags$button(class="btn btn-default", 
			withTags(
              div(
                br(),
                helpText(
				  HTML(paste("<label><h4 align=left><strong>Interpretation :","</strong></h4></label>")),
                  HTML(paste("<p align=justify> Based on above table 1 , we can know the parameter that significant by partial test. 
				  And then table 2 show us the marginal effect of independent variable.</p>"))
					
                ), class = "span20"))),id="col3", value="test3"),
...
#fungsi pemilihan sensor kanan 
 ...
 output$identification<- renderUI({
		...
		if(input$identification_plot=='none'){
			return(tags$button(class="btn btn-danger","Please select plot from dropdown menu to create a plot."))
		}
		if(input$identification_plot=='cen') {
					return(list(a=plotOutput("identification_cen"),b=htmlOutput('ket_cen')))
				}
		else if(input$identification_plot=='hist') {
			return(list(a=plotOutput('hist'),b=htmlOutput('ket_hist')))
		}
		else {
		if(is.null(input$tobit_variableIndep)){
			return(tags$button(class="btn btn-danger","Please select one or more independent variables."))
	...
	output$hist<-renderPlot({
		variable_Dependent_<-data.matrix(variable_Dependent(), rownames.force = NA)
		hist(variable_Dependent_, col="black", xlab=colnames(variable_Dependent_), main="Histogram of Dependent Variable")
	})
	output$ket_hist<-renderText ({
		HTML(paste("<h4 align=justify>Interpretation :</h4>","<p align=justify>Base on the graph above, 
	we can know the most frequency of the censored response variable</p></li>"))
	})
 output$ket_cen<-renderText ({
	HTML(paste("<h4 align=justify>Interpretation :</h4>","<p align=justify>Base on the graph above, 
	we can know the location of the censored response variable</p></li>"))
 })
  
 output$identification_cen<- renderPlot({
		variable_Dependent_<-data.matrix(variable_Dependent(), rownames.force = NA)
		variable_Independent_<-data.matrix(variable_Independent(), rownames.force = NA)
 ...
 output$identification1<- renderPlot({
		...
		data<-data.frame(cbind(dependent_variable,independent_variable))
				...
				if(input$identification_plot=='scat') {
					scatterplot(variable_Dependent(),variable_Independent())
    ...
    namafile <- file_names()
		...
		direktori2 <- paste0("192.168.0.12/fast/images/image/image",namafile,".png")
	...
	inserttodb <- function(direktori = null){
		query<- paste0("INSERT INTO data VALUES(null,'",input$title,"','",input$author,"','",input$category,"','",input$summary,"','",direktori,"')")
 ...
 #fungsi penghitungan uji asumsi formal
   formal_test <- reactive({
					...
					if(input$asumsi_formal=='linear_formal') {
						...
						if(is.null(input$normal_formalchoice)) return()
					...
					if(is.null(input$homo_formalchoice)) return()
					...
					if(is.null(input$nonauto_formalchoice)) return()
			...
			if(is.null(formal_test()$p.value)) return()
		...
		residuals<-residual()
			if(typeof(residual())=="character"){
				...
				if(input$asumsi_plot=='linear_plot') {
					...
					if(is.null(input$normal_plotchoice)) return ()
				else {
