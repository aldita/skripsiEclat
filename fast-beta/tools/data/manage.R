...
output$DataExample <- renderUI({

  # if(isolate(input$datatabs) != 'Manage') return(invisible())

  dat <- as.data.frame(getdata()[1:10,])
  print(dat)
  list(
    tags$br(),
    renderDataTable(dat,class = 'table table-striped table-bordered table-hover', rownames=FALSE, options=list(searching = FALSE, paging=FALSE,dom='t')),
    tags$br()
