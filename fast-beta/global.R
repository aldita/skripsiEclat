...
#EDIT HELP AND REPORT UNTUK SHINY 0.12
helpAndReport2 <- function(title, link, content) {
  html <- sprintf("<div id='%sHelp' class='modal sbs-modal fade' style='display: none; '><div class='modal-size-fix'><div class='modal-content'>
                  <div class='modal-header'><a class='close' data-dismiss='modal' href='#'><span>&times;</span></a>
                  <img src='/image/header.png'>&nbsp;&nbsp;</img><h3>%s</h3>
                  </div>
                  <div class='modal-body'>%s</div>
                  </div></div></div>
                  <div>
                  <a title='Help' data-toggle='modal' href='#%sHelp'><span class='glyphicon glyphicon-question-sign'></span></a>
                  </div> 
                  <div style='clear: both;'></div>
                  ", link, title, content, link, link, link)
  Encoding(html) <- 'UTF-8'
...
inclMD2 <- function(file) return(markdownToHTML(file, options = c(""), stylesheet="www/modalhelp.css"))